...
let express = require('express'),
    ...
    settings = {
        ...
        menu: {
            dashboard_menu: [
                    {link:'Calendar',slug: '/calendar', weight:1, icon:'calendar',subitems:[
                    {link:'Appointments',slug: '/calendar/appointments',min_role:2, weight:2},
                    {link:'Price Settings',slug: '/calendar/price-setting',min_role:2, weight:2}
                ]}
            ]
        }
    },
...
// routes
    ...
    routes.get('/calendar', async (req, res) => {
        ...
        if (req.cookies.selected_date){ // if there's a date that's been previously selected, go to that instead
            ...
            let myslots = {}
        for (var appt of data.appointments){
            if(myslots[appt.staff_id] !== undefined){
                console.log('second')
                myslots[appt.staff_id].push(moment(appt.date).format("HH:mm"))
                myslots[appt.staff_id+'-'+moment(appt.date).format("HH:mm")] = appt
            }else { 
                console.log('first')
                myslots[appt.staff_id] = new Array()
                myslots[appt.staff_id].push(moment(appt.date).format("HH:mm"))
                myslots[appt.staff_id+'-'+moment(appt.date).format("HH:mm")] = appt
            }

            console.log('type',typeof myslots[appt.staff_id])
            if (appt.staff_id == '0000'){
                data.staff.unshift({_key:'0000', name: {first: 'Unassigned',last:'Appointments'}})
                break
            }
        ...
        data.appointment_slots = myslots
    ...
    routes.get('/calendar/:year/:month?/:day?', async (req, res) => {
        ...
        //console.log(req.params,date_string,data.appointments,'data.appointments')
        data.staff = await members.getactivemem() //staff.all()
        data.salon = await salon.find('54855602')
        let myslots = {}
        for (var appt of data.appointments){
            if(myslots[appt.staff_id] !== undefined){
